<template>
  ...
  >
    <div class="caster-display" :style="{ width: `${width}px` }">
      <div
        class="caster-visual layout horizontal center-horizontal center-vertical"
        :style="{ height: `max(${visualHeight}px, 400px)` }"
      >
        <image-loader
          ...
          :src="caster.imageUrl"
        />
        <video-loader v-else :src="caster.videoUrl" class="caster-video" />
      </div>
      <div class="caster-details">
        ...
        <div class="caster-social">
          <span v-if="!isBlank(caster.pronouns)" class="caster-pronouns">
            {{ caster.pronouns }}
          </span>
          <font-awesome-icon
